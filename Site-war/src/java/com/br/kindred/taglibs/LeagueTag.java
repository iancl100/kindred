...
public class LeagueTag extends SimpleTagSupport {
    ...
    public void doTag() throws JspException, IOException {
        ...
        String content = OpenStream.openURL("https://" + region + ".api.pvp.net/api/lol/" + region + "/v2.5/league/by-summoner/" + summonerId + "?");
        League league = null;
        if (content.equals("403")) {
            out.println("<h3>No summoner data found for any specified inputs</h3>");
        }
         else if(!content.equals("404")){
            league = LeagueJSONParser.parserFeed(content, summonerId).get(0);
        }
        if (league!=null && league.getQueue().equals("RANKED_SOLO_5x5")) {
            ...
            for (int i = 1; i <= 50; i++) {
                ...
                if (line.getPlayerOrTeamId().equalsIgnoreCase(summonerId)) {
                ...
                if (line.getLeaguePoints() == 100) {
                    out.println("<section class=\"row " + styleMark + " text-center\" style='margin-bottom:15px;'>"
                            ...
                            + "</section>");
                    if (league.getLines().get(i).getLeaguePoints() != 100) {
                        veri = true;
                    }
                } else {
                    veri = false;
            ...
            out.println("<section class='row text-center'>"
